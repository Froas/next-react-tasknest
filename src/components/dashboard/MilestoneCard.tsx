...
interface MilestoneCardProps {
  ...
  onAddTask?: () => void;
  onMilestoneUpdate?: (data: Partial<Milestone>) => void;
  onMilestoneDelete?: () => void;
}

export default function MilestoneCard({ milestone, goalId, onUpdate, onDelete, onSelectMilestone, onAddTask, onMilestoneUpdate }: MilestoneCardProps) {
  // Store integration
  const {
    addTask,
    updateTask,
  ...
  const handleTaskToggle = async (
    ...
    try {
      ...
      setCurrentMilestone(prevMilestone => {
        ...
        const updatedMilestone = { ...prevMilestone, tasks: updatedTasks };
  ...
  return (
    <div className="bg-card border border-border rounded-lg shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="p-6 border-b border-border">
        <div className="flex items-start justify-between">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mb-3">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-xs text-muted-foreground">{Math.round(calculateProgress())}%</span>
                </div>
          ...
          <div className="flex items-center space-x-2 flex-shrink-0">
            <span className={`px-2 py-1 text-xs font-medium rounded-full border bg-muted text-foreground border-border`}>
              {milestone.status}
            </span>
            <span className={`px-2 py-1 text-xs font-medium rounded-full border bg-accent text-accent-foreground border-border`}>
              {milestone.priority}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="flex items-center space-x-1 px-3 py-1.5 text-sm bg-primary text-primary-foreground rounded-md hover:bg-primary/80 transition-colors"
            >
            ...
            <button
              onClick={onDelete}
              className="p-1.5 text-accent hover:text-accent-foreground hover:bg-accent/10 rounded-md transition-colors"
            >
        ...
        <div className="p-6 bg-muted">
          {isLoading ? (
            <div className="text-center py-8 text-muted-foreground">
              <div className="animate-spin w-6 h-6 border-2 border-muted border-t-primary rounded-full mx-auto mb-2"></div>
              Loading tasks...
            </div>
          ) : tasks.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
      ...
      {isCreatingSubtask && selectedTask && (
        <div className="fixed inset-0 bg-background/80 flex items-center justify-center z-50">
          <div className="bg-card rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold mb-4 text-foreground">Create New Subtask</h3>
            <SubtaskForm
              goalId={goalId}
              milestoneId={milestone.id}
              taskId={selectedTask.id}
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center space-x-2">
              ...
              {task.priority && (
                <span className="px-1.5 py-0.5 text-xs font-medium rounded border bg-accent text-accent-foreground border-border">
                  {task.priority}
                </span>
              )}
            </div>
            {task.description && (
              <p className={`text-xs mt-1 ${isCompleted ? 'text-muted-foreground' : 'text-muted-foreground'}`}>
                {task.description}
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {task.subtasks?.map((subtask) => (
              <div key={subtask.id} className="flex items-center space-x-3 py-1">
                ...
                <span className={`text-sm flex-1 ${subtask.status === StatusType.FINISHED ? 'line-through text-muted-foreground' : 'text-foreground'}`}>
                  {subtask.title}
                </span>
            ...
            {task.todos?.map((todo) => (
              <div key={todo.id} className="flex items-center space-x-3 py-1">
                ...
                <span className={`text-sm flex-1 ${todo.status === StatusType.FINISHED ? 'line-through text-muted-foreground' : 'text-foreground'}`}>
                  {todo.title}
                </span>
