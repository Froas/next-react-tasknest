...
const fieldClass =
  "w-full px-3 py-2 border border-border bg-card text-foreground rounded-lg focus:outline-none focus:ring-2 focus:ring-ring";
...
const TaskFields: React.FC<{
  ...
  <div className="rounded-lg border border-border p-3 space-y-3">
    ...
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      <select
        ...
        onChange={(event) =>
          onChange({
            ...task,
            kind: event.target.value as TemplateDraftTask["kind"],
          })
        }
      ...
      <select
        ...
        onChange={(event) =>
          onChange({
            ...task,
            priority: event.target.value as TemplateDraftTask["priority"],
          })
        }
    ...
    />
    {(task.kind === "project" || task.kind === "challenge") && (
      <div>
        ...
        <textarea
          ...
          onChange={(event) =>
            onChange({ ...task, subtaskTitles: event.target.value })
          }
    ...
    {(task.kind === "routine" || task.kind === "challenge") && (
      <div>
        <label className="mb-1 block text-xs font-medium text-muted-foreground">
          Daily routine items
        </label>
        <textarea
          ...
          onChange={(event) =>
            onChange({ ...task, todoTitles: event.target.value })
          }
          placeholder="One per line or separated by commas"
          rows={2}
        />
...
export const TemplateBuilderModal: React.FC<Props> = ({
  open,
  onClose,
  onCreated,
}) => {
    ...
    setDraft((current) => ({
      ...
      milestones: [
        ...current.milestones,
        {
          title: "",
          description: "",
          dueDateOffsetDays: Math.min(
            current.durationDays,
            Math.max(1, (current.milestones.length + 1) * 14),
          ),
          tasks: [],
        },
      ],
          ...
          <div>
            ...
            <select
              ...
              onChange={(event) =>
                setDraft((current) => ({
                  ...current,
                  priority: event.target.value as TemplateDraft["priority"],
                }))
              }
            >
          ...
          {draft.goalTasks.map((task, index) => (
            <TaskFields
              ...
              onChange={(next) =>
                setDraft((current) => ({
                  ...current,
                  goalTasks: current.goalTasks.map((item, itemIndex) =>
              ...
              onRemove={() =>
                setDraft((current) => ({
                  ...current,
                  goalTasks: current.goalTasks.filter(
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Relative deadlines will start when the template is used.
              </p>
            </div>
            <button
              type="button"
                    ...
                    setDraft((current) => ({
                      ...
                      milestones: current.milestones.map((item, index) =>
                        index === milestoneIndex
                          ? {
                              ...item,
                              tasks: item.tasks.map((taskItem, indexOfTask) =>
                                indexOfTask === taskIndex ? next : taskItem,
                              ),
                            }
                          : item,
                      ),
                        ...
                        index === milestoneIndex
                          ? {
                              ...
                              tasks: item.tasks.filter(
                                (_, indexOfTask) => indexOfTask !== taskIndex,
                              ),
                            }
                          : item,
                      ),
